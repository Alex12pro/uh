...
function rewriteCssUrls(css, baseUrl, origin) {
  return String(css).replace(/url\((['"]?)(.*?)\1\)/gi, (_, q, raw) => {
    ...
    return `url(${q}${proxify(origin, abs(baseUrl, v))}${q})`;
  });
}
...
function rewriteSrcset(srcset, baseUrl, origin) {
  return String(srcset)
    ...
    .map((part) => {
      ...
      return [proxify(origin, abs(baseUrl, rawUrl)), ...pieces].join(" ");
    })
    .join(", ");
}
...
(() => {
  ...
  const proxify = (u) => shouldSkip(u) ? u : \`\${VOID_ORIGIN}/api/go?url=\${encodeURIComponent(abs(u))}\`;

  const navTo = (u) => {
    if (shouldSkip(u)) return;
    const absolute = abs(u);
    if (window.top !== window.self) {
      parent.postMessage({ type: "VOID_NAVIGATE", url: absolute }, "*");
    } else {
  ...
  const rewriteNode = (root) => {
    ...
    root.querySelectorAll("[href],[src],[action],[poster],[data],[srcset],[style]").forEach((el) => {
      ["href","src","action","poster","data"].forEach((attr) => {
        ...
        if (v && !shouldSkip(v)) el.setAttribute(attr, proxify(v));
      });
      ...
      if (ss) {
        const rewritten = ss.split(",").map(part => {
          ...
          if (!raw || shouldSkip(raw)) return seg;
      ...
      if (style && /url\\(/i.test(style)) {
        el.setAttribute("style", style.replace(/url\\((['"]?)(.*?)\\1\\)/gi, (m, q, raw) => {
          return shouldSkip(raw) ? m : "url(" + q + proxify(raw) + q + ")";
        }));
      }
      if (el.tagName === "FORM") {
...
function rewriteHtml(html, baseUrl, origin) {
  ...
  out = out.replace(/<head([^>]*)>/i, `<head$1><base href="${baseUrl}">${injectClient(baseUrl, origin)}`);
  ...
  for (const attr of attrs) {
    ...
    out = out.replace(re, (m, quote, value) => {
      if (shouldSkipUrl(value)) return `${attr}=${quote}${value}${quote}`;
      return `${attr}=${quote}${proxify(origin, abs(baseUrl, value))}${quote}`;
    });
  }
